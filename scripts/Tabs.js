...
class Tabs {
  ...
  updateUI() {
    ...
    this.contentElements.forEach((contentElement, index) => {const isActive = index === activeTabIndex
...
lastTab = () => {
  ...
  onKeyDown = (event) => {
    ...
    const isMacHomeKey = metaKay && code === 'ArrowLeft'
    if (isMacHomeKey) {
      this.firstTab()
      return
    }

    const isMacEndKey = metaKay && code === 'ArrowRight'
    if (isMacHomeKey) {
      this.lastTab()
      return
    }
